...
public class LauncherActions {
	...
	private List<Action> getList() {
		...
		for (int i = 0; i < menuBindingsValues.length; i++) {
			...
			if (value == Launcher.BIND_PREVIEWS || value == Launcher.BIND_NOTIFICATIONS) {
	...
	public static int getIconResourceId(int bindingValue) {
		switch(bindingValue) {
	...
	public class DefaultLauncherAction implements Action {
		...
		public boolean runIntent(Intent intent) {
			if (intent.hasExtra(EXTRA_BINDINGVALUE))
			{
				...
				if (val == mBindingValue) {
					mLauncher.fireHomeBinding(mBindingValue);
					return true;
				}
		...
		@Override
		public int getIconResourceId() {
			return LauncherActions.getIconResourceId(mBindingValue);
		}
	}
}
